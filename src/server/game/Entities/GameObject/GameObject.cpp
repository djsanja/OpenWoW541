    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_FLAGSTAND:                     // 24
        {
            ...
            if (player->CanUseBattlegroundObject(this))
            {
                ...
                return;                                     //we don't need to delete orb2 ... it is despawned!
                bg->EventPlayerClickedOnOrb3(player, this);
                return;                                     //we don't need to delete orb3 ... it is despawned!
                bg->EventPlayerClickedOnOrb4(player, this);
                return;                                     //we don't need to delete orb4 ... it is despawned!
                    ...
                    switch (info->entry)
                    {
                        ...
                        case 184142:                        // Netherstorm Flag
                            if (bg->GetTypeID(true) == BATTLEGROUND_EY)
                                bg->EventPlayerClickedOnFlag(player, this);
                            break;
                        case 210656:                        // ORB OF POWER 1
                            if (bg->GetTypeID() == BATTLEGROUND_TK)
                                bg->EventPlayerClickedOnOrb1(player, this);
                            break;
                        case 210657:                        // ORB OF POWER 2
                            if (bg->GetTypeID() == BATTLEGROUND_TK)
                                bg->EventPlayerClickedOnOrb2(player, this);
                            break;
                        case 210658:                        // ORB OF POWER 3
                            if (bg->GetTypeID() == BATTLEGROUND_TK)
                                bg->EventPlayerClickedOnOrb3(player, this);
                            break;
                        case 210659:                        // ORB OF POWER 4
                            if (bg->GetTypeID() == BATTLEGROUND_TK)
                                bg->EventPlayerClickedOnOrb4(player, this);
                            break;
                    }
